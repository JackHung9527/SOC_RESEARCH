...
### 2026/09/03（論文送印彩色頁判定：逐頁算圖找出需彩印頁；本機 Python 全失效改走 Node 管線）

#### ✅ 完成項目
- **判定論文送印需彩色列印的頁數**（`DOC/離校程序/論文電子檔/鋰電池SOC估測方法之比較與嵌入式實作.pdf`，67 頁），結果寫成 `彩色頁.txt`；使用者已把它與送印用的 PDF、審定書、授權書一起移到 `DOC/離校程序/印論文精裝平裝/`
- **判定方式是逐頁完整算圖後逐像素檢查 RGB 三通道最大差值**（非靠圖說位置或圖檔清單推測），頁數一律以 **PDF 實體頁碼**計，不用頁尾印刷頁碼
- **結果分三層交付**：含彩色共 28 頁／真正需彩印 12 頁（15, 22, 24, 25, 26, 30, 33, 37, 41, 42, 45, 47）／其餘 16 頁（16, 21, 23, 28, 32, 34, 39, 43, 46, 48, 49, 50, 53, 54, 55, 60）僅表格標題列淺藍底，黑白印不影響判讀。純黑白 39 頁
- **抽樣渲染目視覆核分類正確**：P15（圖 2-1 等效電路，藍色元件方塊）、P25（圖 3-3 測試平台全彩照片）、P42（圖 4-4 四倍率彩色散布圖）、P47（圖 4-6 三法軌跡藍綠紅四線）、P16（對照組，僅表頭淺藍底）

#### 🐛 問題與踩坑
- **本機 Python 已全部失效，往後不能再預設有 python**：舊筆記記的 `C:\Users\jackhung\AppData\Local\Programs\Python\Python312\python.exe` 連同整個 `C:\Users\jackhung` 家目錄都不存在了（現行家目錄是 `jackhung.FSP-GROUP`）。兩個 Espressif venv（`tools/python/v5.5.4/venv`、`tools/python_env/idf5.4_py3.12_env`）本體還在，但 base interpreter 指向那條死路徑，一執行就回 `No Python at '...'`，site-packages 裡的 PyMuPDF／pypdf 全部叫不出來。全碟 `find -iname python.exe` 只剩 Windows Store alias
- **改走 Node 管線可完全取代**：`npm i pdfjs-dist @napi-rs/canvas` 一次裝成（公司網路沒擋 npm registry），用 `pdfjs-dist/legacy/build/pdf.mjs` 搭 `createCanvas` 逐頁 render 再讀 `getImageData`，67 頁在 scale=0.7 下數十秒跑完，原本 PyMuPDF 的「算圖＋逐像素」用途一比一對應得上
- **「有非灰像素」不等於「需要彩印」，必須先抽色彩直方圖分群**：首輪 28 頁全判為彩色，但其中 16 頁的最大 RGB 差值都是可疑的同一個值「恰好 26」，抽主色一看是 `#DEEAF6`——Word 表格標題列的預設淺藍網底。真正的彩圖頁差值落在 108～222。少了這一步會叫使用者多付一倍以上的彩印費
- **用「單頁最大 RGB 差值」分群比用「彩色像素數」乾淨**：淺藍表頭的彩色像素數動輒上千，比某些真彩圖還多（p49 有 14635 px 卻只是表頭），單看數量分不開；改看差值後 26 與 108 之間有一條完全乾淨的界線，零誤判
- **Bash heredoc 即使用 `<<'EOF'` 引號版，仍會吃掉 JS 字串裡的反斜線**：模板字串裡寫兩個反斜線接 `p` 期望產出 `png\p15.png`，實際被解成單一 `\p` → `p`，檔案落成 `pngp15.png`（目錄名直接黏在檔名前），**而且不報錯、只是靜默寫錯位置**。與 2026/08/03 那條「heredoc 內字面 `\n` 被吃掉」同一根因：**heredoc 傳含反斜線的程式碼一律改用 Write 工具落檔**
- pdfjs 會噴 `standardFontDataUrl` 與 `TT: undefined function` 警告，字型沒載齊不影響「有無顏色」的判斷，可忽略

#### 📋 待辦
- 送印時把 `彩色頁.txt` 第二段那 12 頁交給印刷廠即可；若廠商要「凡有顏色一律彩印」再改用 28 頁那組
- 8/31 遺留項延續：三個前置頁標題 24 pt（參考檔為 20 pt）是否統一、節／小節標題是否比照章標題置中、TOC 不粗體在使用者按 F9 全部更新後會失效（需把節標題粗體由 direct formatting 改成具名樣式，涉及 60 個段落）
...
#### ✅ 完成項目
- **修好離校程序論文電子檔的目錄只有章、沒有節的問題**（`DOC/離校程序/論文電子檔/鋰電池SOC估測方法之比較與嵌入式實作.docx` 就地覆蓋，原始版本備份於 scratchpad）：60 個節標題（28 個 `x.y` 節、32 個 `x.y.z` 小節）補上大綱層級，主目錄條目由 12 條擴充為 72 條（章層級 12／節 28／小節 32）
- **只加大綱屬性、不改樣式也不動字級**，因此內文外觀完全零變化；14 pt／13 pt 粗體的節標題在正文中維持原樣
- **圖目錄／表目錄改掛新建的 `toc 4` 樣式**：原本兩份清單借用 `toc 2` 排版，而 `toc 2` 本次要讓給節條目；新增 `toc 4` 完整複製原 `toc 2` 定義（點狀引線 8900、右縮排 480、標楷體 11 pt），欄位碼改為 `\t "圖標題,4"`／`"表標題,4"`，圖表目錄視覺零變動、仍為 12 圖／11 表
- **`toc 2`／`toc 3` 排版調整**：節縮排 240、小節縮排 480，兩者都加右縮排 480 與 8900 位置的點狀引線定位點（沿用先前「文字界 < 定位點 < 版心」的原則避免長標題換行後出現整行純點），並移除 `toc 3` 的 `<w:autoRedefine/>` 防止 Word 用實際排版回寫污染樣式定義
- Word COM 全自動更新三份目錄欄位並重算頁碼（非估算），全文 67 頁；使用者自行在 Word 做最終確認並重出 PDF

#### 🐛 問題與踩坑
- **根因不在欄位碼，查錯順序決定成敗**：直覺會先懷疑 `TOC \o "1-1"` 之類的層級設定（8/25 才踩過 stale field result），但這次抽出欄位碼一看本來就是 `TOC \o "1-3"`、設定完全正確。真因是節標題全是 `Normal` 樣式配 direct formatting（14 pt／13 pt 粗體），**沒有 `outlineLvl`**，在 Word 眼中只是一般內文。**方法上的教訓：欄位碼與段落大綱層級是兩個獨立的必要條件，必須兩邊都讀過再下判斷**，只查其中一邊會走冤枉路
- **`body.iter(w('p'))` 會掃進表格內的段落**：第一版用 `iter` 遍歷，命中 146 段（預期 60），因為表格儲存格裡的數據如「4.1」「2.2」「1.77」全被節號 regex 命中。改用 `body.findall(w('p'))` 只掃 body 直屬段落後正好 60 段。**先印出待改清單逐條目視、並用 `assert` 卡住預期數量，是這次沒有誤傷表格的關鍵**——若直接寫檔，146 段的大綱層級會讓目錄塞滿表格內的數字
- **圖表目錄與節條目搶同一個 `toc 2` 樣式**：直接給 `toc 2` 加左縮排會連帶把圖表目錄一起改掉（使用者沒要求的副作用）。修法是新增 `toc 4` 承接圖表目錄、`toc 2` 專供節條目，兩邊互不干擾。**`\t "樣式名,層級"` 的層級數字只是指定套用第幾層 toc 樣式，可以自由改指到不衝突的層級**
- **`outlineLvl` 在 `CT_PPr` 有 schema 位置要求**：須排在 `jc` 之後、`rPr`／`sectPr`／`pPrChange` 之前，插錯位置會產生不合法 XML。實作上是找第一個屬於「outlineLvl 之後」集合的子元素做 `addprevious`，找不到才 append
- **Word COM 本次可用**：8/25 記錄的 `80040154 Class not registered` 未再出現，`New-Object -ComObject Word.Application` 正常回 16.0，故頁碼得以全自動重算，不必再走 PDF 逐頁抽字對齊印刷頁碼那條迂迴路線。**該錯誤應屬當時的環境瞬時狀態，不是這台機器的固有限制，日後仍應先試 COM**
- **`ExportAsFixedFormat` 出 PDF 在 6 MB／67 頁的文件上遠比存檔慢**，兩度逾時進背景；由於 `Save()` 排在匯出之前，docx 內容其實早已落地，中止匯出不影響交付檔正確性。收尾時只終止自己啟動的那個隱形 Word 程序（以啟動時間辨識，避免誤殺使用者自己開的兩個），再清掉 `~$` 鎖檔
- **`git pull --ff-only` 在 my-claude-extensions 報 `Cannot fast-forward to multiple branches`**：不是衝突也不是分岔（實測 0 ahead／1 behind），而是本機殘留一個 upstream 已 gone 的 `claude/web-claude-extensions-59an38` 分支導致無參數 pull 混淆。明確指定 `git pull --ff-only origin main` 即通

#### 📋 待辦
- 該分支 `claude/web-claude-extensions-59an38` 的 upstream 已 gone，可考慮清掉以免每次 pull 都要指定分支
- 論文既有待辦延續（表 4-4 圖說缺字、第二～四章 8 個未編號表格未進表目錄、第四章從 4.0 起編、封面姓名／指導教授佔位、方程式編輯器重排、圖 3-3 測試照片補入後重匯 PDF、13 篇付費文獻待圖書館下載）

### 2026/08/25（依他人論文格式重排目錄／圖表目錄／參考文獻；建立 docx＋PDF 頁碼抽取管線）

#### ✅ 完成項目
- **依使用者提供的他人論文（OpenROAD／DQN 主題）版式，把本論文的目錄、圖目錄、表目錄、參考文獻改排成同一種樣式，全部以純文字交付**：使用者明確選擇「只要純文字整理，不動任何檔案」，故論文 docx／PDF 完全未改
- **目錄補到節層級**：原 docx 的 Table of Contents 只有章層級 6 條，新版補齊 1.1–6.5 共 34 條；章名改「第X章、名稱」、節號後改全形空格
- **節層級頁碼由實檔抽出而非估算**：docx 內沒有節層級頁碼快取，改由 `_20260803.pdf` 逐頁抽文字、以頁首印刷頁碼對齊（實體頁 8 = 印刷頁 1，offset 7），再用整行完全比對定位每個標題；章層級結果與 docx 目錄快取的 6 筆頁碼完全一致＝交叉驗證通過
- **參考文獻 33 篇改寫成範例的 IEEE 風格**：頁碼範圍連字號改 en dash、研討會條目刪城市國別、刪月份、作者超過 6 位改「第一作者 et al.」（僅影響 [27] Song 與 [33] Yi）、會議名稱展開不用縮寫
- **圖表編號兩版都給**：先出範例式連續編號（圖 1–12／表 1–11）並附新舊對照表，使用者選擇改回章節式，最終交付 圖 2-1～圖 4-6／表 4-1～表 6-1 版本，與內文引用完全一致、內文不需改動
- **圖說一律裁到冒號前、移除長括號註解**，比照範例的一行式圖說；完整圖說仍留在正文
- 工作區既有的 `DOC/離校程序/` 重整一併提交：拆成 `注意事項/`、`給老師簽名(論文比對結果 , 授權書 , 離校手續單)/`、`論文電子檔/` 三個子資料夾，並新增口試委員審定書、論文授權書、程序.txt 等檔

#### 🐛 問題與踩坑

**論文本身查出的問題（本次只回報，未修改任何檔案）**
- **表 4-4 圖說缺字**：原文「動態阻抗離線為 實測）」，「為」與「實測」之間空一格，應是漏字
- **8 個表格沒有編號圖說，因此進不了表目錄**：全文共 19 個表格物件、只有 11 個有編號（第二章 2 個、第三章 5 個含圖 3-3 照片框那個 1×1 表格、第四章 1 個未編號）
- **第四章從 4.0 起編**，其餘各章都從 x.1 起
- **只有第四、五章有「本章小結」**，第一、二、三、六章沒有

**流程踩坑**
- **Word COM 在本環境不可用**：`New-Object -ComObject Word.Application` 直接回 `80040154 Class not registered`，無法沿用過去用 Word COM 取頁碼的做法。改走 pypdf 讀 PDF，反而更快也不必開 Word
- **本機三個 python 都沒有 pypdf／python-docx／fitz**，全碟掃描只有 `C:\Espressif\tools\python\python.exe` 一個可用 python；`pip install pypdf` 可通（純 python 套件、公司網路沒擋）
- **Windows python 不認 `/tmp`**：腳本裡寫 `/tmp/pages.json` 直接 FileNotFoundError，要改用 `os.environ['TEMP']`
- **`<w:t[^>]*>` 這個 regex 會誤中 `<w:tab w:val="left"/>`**（`<w:t` ＋ `ab w:val=...` ＋ `/>`），導致抽段落文字時把整段 XML 原始碼吐出來。要用 `<w:t\b[^>]*>`，`\b` 在 `w:tab` 的 t 與 a 之間不成立才擋得掉。抽參考文獻時沒事（那些段落沒有 tab），一抽目錄就爆
- **標題定位不能只用「該頁含此字串」**：1.4 論文架構的內文逐條列出「第二章 文獻回顧：…」，比對「第二章 文獻回顧」會先命中前一頁；目錄頁本身也含章名。修法是逐行完全比對（去空白後 `==`）並從正文首頁起算
- **PDF 抽出的文字會被塞入多餘空格**（例：`Open-Circuit V oltage`），所以比對前一律 `re.sub(r'\s+','',s)` 去掉全部空白再比

#### 📋 待辦
- 表 4-4 圖說缺字待補
- 第二、三、四章那 8 個未編號表格，若要進表目錄需補編號圖說（會連帶影響既有 表 4-x 編號）

...
Uart_debug_TaskSel
    g_uart_debug_taskSel = Uart_debug_TaskSel_TaskAwait;
Uart_debug_FlowSel
    g_uart_debug_flowSel = Uart_debug_FlowSel_FlowAwait;

uint32_t
    g_uart_debug_cmd     = _timxTick_cmd_start,
    g_uart_debug_cnt     = 0;


/* ===== TX ring buffer ===== */
static uint8_t  s_tx_buf[UART_DEBUG_TX_BUF_SIZE];
...
/* ===== RX line buffer ===== */
static char     s_rx_line[UART_DEBUG_RX_LINE_SIZE];
...
static Uart_debug_RxLineCb s_rx_cb = NULL;


/* ===== private prototypes ===== */
static void uart_debug_kick_tx(void);
 ...
 */
void uart_debug_handle(void)
{
    if (s_rx_line_ready)
    {
        if ((g_uart_debug_taskSel == Uart_debug_TaskSel_TaskAwait) &&
            (g_uart_debug_flowSel == Uart_debug_FlowSel_FlowAwait))
        {
            g_uart_debug_taskSel = Uart_debug_TaskSel_Service_Routine;
            g_uart_debug_flowSel = Uart_debug_FlowSel_RxDispatch;
        }
    }

    uart_debug_TASK(&g_uart_debug_taskSel, &g_uart_debug_flowSel);
}
 ...
 */
uint16_t uart_debug_send(const uint8_t *data, uint16_t len)
{
    uint16_t written = 0;

    while (written < len)
    {
        uint16_t next_head = (uint16_t)((s_tx_head + 1U) % UART_DEBUG_TX_BUF_SIZE);
 ...
 * @return 實際送出的 byte 數
 */
int uart_debug_printf(const char *fmt, ...)
{
    char tmp[UART_DEBUG_TX_BUF_SIZE / 2];
    va_list ap;
    va_start(ap, fmt);
    ...
    return (int)uart_debug_send((const uint8_t *)tmp, (uint16_t)n);
}
 ...
 */
uint16_t uart_debug_tx_free(void)
{
    uint16_t head = s_tx_head;
    uint16_t tail = s_tx_tail;
    if (head >= tail) return (UART_DEBUG_TX_BUF_SIZE - 1U) - (head - tail);
    return (tail - head) - 1U;
}
                          ...
                          Uart_debug_FlowSel *flow)
{
    switch ((int)*task)
    {
        case Uart_debug_TaskSel_Service_Routine:
        {
            switch ((int)*flow)
            {
                case Uart_debug_FlowSel_RxDispatch:
                {
                    if (s_rx_line_ready)
                    {
                        if (s_rx_cb != NULL)
                    ...
                    *flow = Uart_debug_FlowSel_finish;
                    break;
                }
                case Uart_debug_FlowSel_finish:
                {
                    *task = Uart_debug_TaskSel_TaskAwait;
                    *flow = Uart_debug_FlowSel_FlowAwait;
                    break;
                }
                default:
                    *task = Uart_debug_TaskSel_TaskAwait;
...
static void uart_debug_kick_tx(void)
{
    ...
    if (!s_tx_busy && (s_tx_head != s_tx_tail))
    {
        ...
        s_tx_tail = (uint16_t)((s_tx_tail + 1U) % UART_DEBUG_TX_BUF_SIZE);
        s_tx_busy = 1;
        __enable_irq();
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    if (!s_rx_line_ready)
    {
        ...
        else if (s_rx_len < (uint16_t)(UART_DEBUG_RX_LINE_SIZE - 1U))
        {
...
{
    (void)file;
    return (int)uart_debug_send((const uint8_t *)ptr, (uint16_t)len);
}
